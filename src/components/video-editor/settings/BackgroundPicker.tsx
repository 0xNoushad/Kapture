...
export function BackgroundPicker({ selected, onWallpaperChange, videoElement }: BackgroundPickerProps) {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (wasPlayingRef.current && videoElement) {
    ...
    reader.onerror = () => toast.error('Failed to upload image', { description: 'There was an error reading the file.' });
    ...
    <Tabs defaultValue="image" className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-2">
        <TabsContent value="image" className="mt-0 space-y-3 px-2">
          ...
          <Button onClick={handleUploadClick} variant="outline" className="w-full gap-2 bg-white/5 text-slate-200 border-white/10 hover:bg-[#34B27B] hover:text-white hover:border-[#34B27B] transition-all">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isSelected ? "border-[#34B27B] ring-2 ring-[#34B27B]/30 scale-105 shadow-lg shadow-[#34B27B]/10" : "border-white/10 hover:border-[#34B27B]/40 hover:scale-105 opacity-80 hover:opacity-100 bg-white/5"
                  )}
        ...
        <TabsContent value="color" className="mt-0 px-2">
          <div className="p-1">
            <Block
              ...
              onChange={(color) => { setSelectedColor(color.hex); onWallpaperChange(color.hex); }}
          ...
          <div className="grid grid-cols-6 gap-2.5">
            {GRADIENTS.map((g, idx) => (
              <div
                ...
                className={cn(
                  "aspect-square w-12 h-12 rounded-md border-2 overflow-hidden cursor-pointer transition-all duration-200 shadow-sm",
                  gradient === g ? "border-[#34B27B] ring-2 ring-[#34B27B]/30 scale-105 shadow-lg shadow-[#34B27B]/10" : "border-white/10 hover:border-[#34B27B]/40 hover:scale-105 opacity-80 hover:opacity-100 bg-white/5"
                )}
                style={{ background: g }}
